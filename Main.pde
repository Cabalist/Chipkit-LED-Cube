/* The tab has the loop where you can build your own application.  The sample one here contains two demos. 
 The first moves a light around in the cube, changing its color gradually as it goes, demonstrating the
 190 color "color wheel".  The second one simply interrupts the first one periodically to show the 
 standard named colors, which can be called by name. You can replace this demo with your own application.
 */
...
void loop() {
  ...
  for (byte layer=0; layer<5; layer++){  // scan thru each layer
    for (byte panel=0; panel<8; panel++){  // scan thru each panel
      if (panel==0){
        getColor(Red,layer);  //Load Standard Red
      }
      if (panel==1){
        getColor(Orange,layer);   //Load Standard Orange
      }
      if (panel==2){
        getColor(Yellow,layer);   //Load Standard Yellow
      }
      if (panel==3){
        getColor(Green,layer);   // Etc.  
      }
      if (panel==4){
        getColor(Aqua,layer);   // Intensity is set to layer, so bottom 5 layers display the different intensities
      }                         // with bottom layer off (0 intensity)
      if (panel==5){
        getColor(Blue,layer);
      }
      if (panel==6){
        getColor(Violet,layer);
      }
      if (panel==7){
        getColor(Purple,layer);
      }
      LED(7, panel, layer, myred, mygreen, myblue);// transfer them all to column 7 of the cube for display. 
  ...
  for (byte panel=0; panel<8; panel++){  // scan thru each panel
    for (byte layer=0; layer<5; layer++){  // scan thru each layer
      LED(7, panel, layer, 0, 0, 0);// clear column 7
    }
  }
}
