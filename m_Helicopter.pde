/*  This is the helicoper animation. This would be better if our sprite could be bigger, specifically longer, than 
our 4x4x4 maximum sprite size allows, but we did the best we could with the space available.  

There are a couple of subroutines that are external to the primary one, so we define our helocopter sprite outside 
of the primary routine, as well as a couple counter variables that we need to be global.  This animation denostrates how 
you can use a sprite, but then modify its appearance on the fly during the animation, as we do with the rotor blade 
and the blicking tail light. */
