...
void scrollOneChar(int xRed, int xGreen, int xBlue, int Mode) {
    for (int shift = 0;
         shift < 8; shift++) { // Before going to the next character, display the content of the text buffer
        for (int col = 0; col <
                          45; col++) {     // eight times, each time shifting the content of the text buffer to the left by one LED.
            for (int row = 0; row < 8; row++) {
                text_buffer[col][row] = text_buffer[col + 1][row];
            }
        }
        for (int col = 0;
             col < 8; col++) {  // Here the text buffer is actually displayed across the 4 walls of the cube
            for (int row = 0; row < 8; row++) {
