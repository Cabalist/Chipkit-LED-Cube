...
void readmsgeq7(int VUtop[7], int VUtopb[8], int VU[7]) {
    int GAIN;
//int virtualGain = 127; // define initial gain status until it is read from A5
// int threshold = 900; // this is the threshold from 0 to 1023 of how high the bass must be to consider it a "beat"
    ...
    for (byte mfreq = 0; mfreq < 7; mfreq++) { // read 7 frequencies
        ...
        VUtop[mfreq] = min(VU[mfreq] / virtualGain, 8); // make sure VUtop is not higher than 8 if gain if high and input is high
...
// Create alternate VUtop array to include a phantom frequency between 0 and 1 for those effects using 8 frequencies.
    VUtopb[0] = VUtop[0];
    VUtopb[1] = min((VU[0] + VU[1]) / (virtualGain + virtualGain), 8);
    for (byte mfreq = 2; mfreq < 8; mfreq++)
        VUtopb[mfreq] = VUtop[mfreq - 1];
    ...
    if (GAIN > 8) { virtualGain = 128 - (GAIN / 8); } // when analog pin is not used, leave the virtualGain alone - otherwise make it a ## from 1 to 128 subtracted from the gain.

    // if (VU[1] < threshold){beat = 0;} // beat detect routine
    // if (VU[1] > threshold){beat = 1;} // beat detect routine
    // the variable beat was removed since we're not using it.   
}
