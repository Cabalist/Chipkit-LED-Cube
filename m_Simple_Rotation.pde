/* This method of rotation is simple in concept, though a little messy to impliment. It rotates a 
   single plane around in the cube.  This single is initally placed on the diagonal. It uses the table
   below (called table) to determine the position of y for each x though a simple 45 degree transition
   The table is used in reverse to get through the next 45 degreees.  These steps need to be repeated 
